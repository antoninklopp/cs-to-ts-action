...
async function run() {
	try {
		...
		const changedFiles: changedFile[] = await getChangedFiles(client, prNumber)
...
async function getChangedFiles(
	...
	const changedFiles = listFilesResponse.data.map(f => { return { name: f.filename, contentUrl: f.contents_url } });
